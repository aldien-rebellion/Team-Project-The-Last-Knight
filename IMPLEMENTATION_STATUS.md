...
- The first Church boss attempt ended in death because the probe spent attack cooldowns on late stagger hits, missing subsequent parries. A focused Church run, reserving swings for each windup, defeated MoonstoneKeeper with HP 100, earned ChurchKey and 100 gold, and released the barriers. Arthur traversed the stairs, used F at Church_1 to arrive upstairs, walked to x=76.4, and used F to open the chest and acquire Rune 1. Inspected `Captures/church-rune1-physical-route.png`; console returned zero errors. This focused run started in Church, so a single uninterrupted complete route is still outstanding.
